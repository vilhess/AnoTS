...
class TranAD(nn.Module):
    ...
    def forward(self, x, target):
        ...
        c = (x1 - target)**2
        target, memory = self.encode(x1, c, target)
        x2 = self.decoder1(target, memory)
        x2 = self.fc_out(x2)
        
        return x1, x2
    
class CatEmbedding(nn.Module):
    def __init__(self, cat_dims=[7, 2], embed_dims=2):
        super().__init__()
        self.embeds = nn.ModuleList([
            nn.Embedding(num_embeddings=in_dim, embedding_dim=embed_dims)
            for in_dim in cat_dims
        ])

    def forward(self, x_cat):
        x_cat = x_cat.permute(2, 0, 1)
        embedded = []
        for cat, embed in zip(x_cat, self.embeds):
            cat = cat.int()
            e = embed(cat)
            embedded.append(e)

        return torch.cat(embedded, dim=2)