...
class BottleNeck(nn.Module):
    def __init__(self, num_cont_var=4, embedding_dim=4, hidden_size=128, latent_size=20):
        ...
        self.fc2_h = nn.Linear(in_features=latent_size, out_features=2*embedding_dim + num_cont_var)

        self.fc1_c = nn.Linear(in_features=hidden_size, out_features=latent_size)
        self.fc2_c = nn.Linear(in_features=latent_size, out_features=2*embedding_dim + num_cont_var)
...
class Decoder(nn.Module):
    def __init__(self, num_cont_var=4, embedding_dim=4, hidden_size=128, num_layers=1, bidirectional=False):
        ...
        self.lstm = nn.LSTM(input_size=hidden_size,  hidden_size=2*embedding_dim + num_cont_var, num_layers=num_layers, batch_first=True, bidirectional=bidirectional)
